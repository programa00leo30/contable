...
;(function ( $, window, document, undefined ) {
    ...
    var pluginName = "YellowText",
        defaults = {
          ...
          defaultActions   : ["bold", "underline", "italic", "strikethrough", "align-left", "align-center", "align-right", "unordered-list", "ordered-list", "link", "image","source"],
      ...
      createButtons: function() {
        ...
        for( var i = 0; i < this.options.defaultActions.length; i++ ) {
          ...
          switch( this.options.defaultActions[i] ) {
            case "bold" :
              button = { content : "bold", command : "bold" };
            break;
            case "underline" :
              button = { content : "text-width", command : "underline" };
            break;
            case "italic" :
              button = { content : "italic", command : "italic" };
            break;
            case "strikethrough" :
              button = { content : "s", command : "strikethrough" };
            break;
            case "align-left" :
              button = { content : "align-left", command : "JustifyLeft" };
            break;
            case "align-center" :
              button = { content : "align-center", command : "JustifyCenter" };
            break;
            case "align-right" :
              button = { content : "align-right", command : "JustifyRight" };
            break;
            case "unordered-list" :
              button = { content : "list", command : "InsertUnorderedList" };
            break;
            case "ordered-list" :
              button = { content : "list", command : "InsertOrderedList" };
            break;
            case "image" :
              button = { content : "picture", command : "image" };
            break;
            case "link" :
              button = { content : "link", command : "link" };
            break;
            case "source" :
              button = { content : "source", command : "source" };
            break;
            default :
              button = { content : "option-horizontal", command : "" };
          }

          // Build the buttons and add before the container
          // $("<span />").addClass( "glyphicon glyphicon-" + button.command ).text( button.content ).data( "command", button.command ).appendTo( this.buttons );
          $("<span />")
			.addClass( "glyphicon glyphicon-" + button.content )
			.data( "command", button.command )
			.appendTo( this.buttons );
          /* 
           $("<a />")
            	.addClass(button.content)
            	 .text( "<p class=\"glyphicon glyphicon-" + button.content + "\" />" )
            	 .data( "command", button.command )
            	 .appendTo( this.buttons );
			*/
			$ ("<a />")
      ...
      runCMD: function( cmd ) {
        ...
        if( cmd === "image" ) {
          ...
          return this.editor.contentWindow.document.execCommand( "CreateLink", false, link);
			...
			$( this.element ).show();
			 this.editor.hide() ;// $( this.element ).hide();
        } else {
